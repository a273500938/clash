...
func Pack(dst, plaintext []byte, s shadowstream.Cipher) ([]byte, error) {
...
func Unpack(dst, pkt []byte, s shadowstream.Cipher) ([]byte, error) {
	if len(pkt) < s.IVSize() {
...
func NewPacketConn(c net.PacketConn, ciph shadowstream.Cipher) *PacketConn {
